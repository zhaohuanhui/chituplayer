...
import java.util.List;
    ...
    CallbackInterface callbackInterface;
        private List<HistoryRecordBean> mData;
        private Context context;
        private int editeTotal=0;
        public HistoryRecordAdapter(List<HistoryRecordBean> mData,Context context){
            this.mData = mData;
            this.context=context;

        }
        public void refresh(List<HistoryRecordBean> data){
            this.mData = data;
        ...
        @NonNull
        @Override
        public HistoryRecordAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View contView = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_favorites,parent,false);
            return new HistoryRecordAdapter.ViewHolder(contView);
        }

        @Override
        public void onBindViewHolder(@NonNull HistoryRecordAdapter.ViewHolder holder, int position) {
//        holder.textView.setText(mData.get(position));
            HistoryRecordActivity activity = (HistoryRecordActivity) context;
